...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "enter":
			selectedItem, ok := m.list.SelectedItem().(DockerCompose)
			if ok && !m.showSpinner {
				m.choice = string(selectedItem)
	...
	case processMsg:
		...
		for i, itm := range m.composes {
			if itm.Index == m.activeItem.Index {
				m.composes[i] = DockerCompose{Index: itm.Index, Path: itm.Path, Status: itm.Status, Config: itm.Config, Commands: itm.Commands}
			}
		}

		listItems := make([]list.Item, len(m.composes))
		for i, itm := range m.composes {
			listItems[i] = itm
		}
